async function load_volume_avc(url, size)
{
    return new Promise((resolve, reject) => {
        let decoder = new Decoder({rgb: false});    
        let arr3d = new ArrayBuffer(size.x * size.y *size.z);
        let view3d = new Uint8Array(arr3d);
        
        let size_slice = size.x * size.y;
        let count =0;
        decoder.onPictureDecoded = (buffer, width, height, infos) =>{
            let y = buffer.subarray(0, size_slice);
            view3d.set(y, count * size_slice);
            count++;
            if (count == size.z)
            {
                resolve(arr3d);
        ...
        (async ()=>{    
            ...
            let offset = 0;
            while (offset<arrBuf.byteLength)
            {
                let size16 = data_view.getUint16(offset, true);
                offset+=2;
                let isKey = (size16 & (1<<15)!=0);
                size16 &= ~(1<<15);
                
                let pkt = u8_view.subarray(offset, offset + size16);
                decoder.decode(pkt);
...
export async function test()
{
    ...
    let vol_size = new Vector3(208,256,225);
    let arr3d = await load_volume_avc("cthead.avc", vol_size);    
    let volData = VolumeDataLoader.LoadVolume(arr3d, vol_size);
